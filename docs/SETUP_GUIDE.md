...
## Пример multi-account маппинга

Пример того, как выглядит маппинг Firefox-профиль → аккаунт при нескольких ботах на разных аккаунтах (реальные email/proxy/credentials — в приватной заметке, не в git):

| Профиль | Аккаунт | Назначение |
|---|---|---|
| `profile-1.claimbot` | Account 1 | Основной набор ботов |
| `profile-2.default` | Account 2 | Второй набор ботов |
| `profile-3.claimbot` | Account 3 | Отдельная стратегия |
